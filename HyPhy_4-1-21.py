...
maxloadeddata=5 #note: 3/20/21 changed from 20 to 5 bc memory issues. 3/2/21 must make a global var in order to print what is actually passed to q_tr
...
adam = optimizers.Adam(clipnorm=0.9,lr=0.1*10**(-3), beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
...
logger = CSVLogger("./log/csv_log_z_4-1-21.csv", separator=',', append=True)
...
hi = Hy.cvae.fit_generator(generator=q_tr,
         shuffle=True,
            verbose=1,
            steps_per_epoch=40, #note: 4/2/20 40 gets all the data
            epochs=10,  #note: 40spE * 35E = 11.5hr, maintaining spE=40 for running through all the data, we can vary E, 1E=
...
plt.colorbar()
plt.savefig(f'x_decoded_{datetime.now()}.pdf',bbox_inches='tight',format='pdf')
plt.show()
plt.title('x')
plt.imshow(x[0][0][0][0][10:-10,10:-10])
plt.colorbar()
plt.savefig(f'x_{datetime.now()}.pdf',bbox_inches='tight',format='pdf')
plt.show()
